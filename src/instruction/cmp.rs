...
// instruction! {@partial if_icmpeq, 0x9f, i16}
// instruction! {@partial if_icmpne, 0xa0, i16}
// instruction! {@partial if_icmplt, 0xa1, i16}
// instruction! {@partial if_icmpge, 0xa2, i16}
// instruction! {@partial if_icmpgt, 0xa3, i16}
// instruction! {@partial if_icmple, 0xa4, i16}
...
// impl InstructionAction for if_icmpeq {
//     fn exec(&self, frame: &mut StackFrame, _: &mut JVM) {
//         let Self(jmp) = *self;
//
//         let val2 = frame.stack.pop().unwrap();
//         let val1 = frame.stack.pop().unwrap();
//         let order = val1.partial_cmp(&val2).expect("Unable to get ordering for branching");
//
...
impl InstructionAction for ifnonnull {
    fn exec(&self, frame: &mut StackFrame, _: &mut JVM) {
        ...
        if let Some(LocalVariable::Reference(Some(_))) = frame.stack.pop() {
            debug!("Branching by {}", jmp);
            frame.branch_offset += jmp as i64;
        }
    }
...
impl InstructionAction for ifnull {
    fn exec(&self, frame: &mut StackFrame, _: &mut JVM) {
        ...
        if let Some(LocalVariable::Reference(None)) = frame.stack.pop() {
            debug!("Branching by {}", jmp);
            frame.branch_offset += jmp as i64;
        }
    }
